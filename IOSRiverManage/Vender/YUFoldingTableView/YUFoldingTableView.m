...
- (void)yuFoldingSectionHeaderTappedAtIndex:(NSInteger)index
{
    ...
    if (rowArray.count) {
        if (currentIsOpen) {
            ...
            NSLog(@"delete %f,%f", self.frame.size.height, self.contentSize.height);
        }else{
            [self insertRowsAtIndexPaths:[NSArray arrayWithArray:rowArray] withRowAnimation:UITableViewRowAnimationTop];
            NSLog(@"add %f,%f", self.frame.size.height, self.contentSize.height);
            
        }
        
        if (_foldingDelegate && [_foldingDelegate respondsToSelector:@selector(yuFoldingTableView:tapHeader:)]) {
            
            [_foldingDelegate yuFoldingTableView:self tapHeader:self.contentSize.height];
