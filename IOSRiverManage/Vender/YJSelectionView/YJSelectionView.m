...
- (UILabel *)descriptionLabel {
    if (!_descriptionLabel) {
        ...
        NSLayoutConstraint *width = [NSLayoutConstraint constraintWithItem:_descriptionLabel attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeWidth multiplier:1.0 constant:70];
...
- (UILabel *)detailLabel {
    if (!_detailLabel) {
        ...
        [_detailLabel setTextColor: [UIColor colorWithRed:51/255.0 green:51/255.0 blue:51/255.0 alpha:1/1.0]];
        _detailLabel.translatesAutoresizingMaskIntoConstraints = NO;
        [self addSubview:_detailLabel];
        NSLayoutConstraint *centerY = [NSLayoutConstraint constraintWithItem:_detailLabel attribute:NSLayoutAttributeCenterY relatedBy:NSLayoutRelationEqual toItem:self attribute:NSLayoutAttributeCenterY multiplier:1.0 constant:0];
        NSLayoutConstraint *leading = [NSLayoutConstraint constraintWithItem:_detailLabel attribute:NSLayoutAttributeLeft relatedBy:NSLayoutRelationEqual toItem:self.descriptionLabel attribute:NSLayoutAttributeRight multiplier:1.0 constant:0];
        NSLayoutConstraint *trailing = [NSLayoutConstraint constraintWithItem:_detailLabel attribute:NSLayoutAttributeTrailing relatedBy:NSLayoutRelationGreaterThanOrEqual toItem:self attribute:NSLayoutAttributeTrailing multiplier:1.0 constant:45];
        [self addConstraint:leading];
        [self addConstraint:trailing];
        [self addConstraint:centerY];
    }
...
+ (YJSelectionView *_Nonnull)showWithTitle:(NSString *_Nonnull)title memoryAddress:(NSString *_Nullable)memory options:(NSArray *_Nonnull)optionsArray singleSelection:(BOOL)selection delegate:(id _Nonnull )delegate completionHandler:(CompleteSelection _Nonnull)handler{
    
    YJSelectionView *view = [[YJSelectionView alloc] initWithFrame:kRootWindow.bounds];
    [kRootWindow addSubview:view];
    view.memoryAddress = memory;
//    [view clearMemory];
    view.selectedArray = [[[NSUserDefaults standardUserDefaults] objectForKey:[NSString stringWithFormat:memory, delegate]] mutableCopy];
    if (!view.selectedArray) {
        view.selectedArray = [NSMutableArray new];
    }
    view.isSingleSelection = selection;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSArray *tempArray = [self.optionsArray[indexPath.row] componentsSeparatedByString:@" "];
    cell.descriptionLabel.text = tempArray.firstObject;
    
    cell.detailLabel.text = tempArray[1];
...
- (void)clearMemory{
    ...
    [[NSUserDefaults standardUserDefaults] removeObjectForKey:_memoryAddress];
    [[NSUserDefaults standardUserDefaults] synchronize];
}
