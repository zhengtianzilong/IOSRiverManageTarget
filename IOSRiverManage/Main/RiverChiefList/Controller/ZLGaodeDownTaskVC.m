...
@interface ZLGaodeDownTaskVC ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation ZLGaodeDownTaskVC
...
- (void)getuserListData{
    ...
    ZLGetUserListByTaskNormalService *service = [[ZLGetUserListByTaskNormalService alloc]initWithriverCode:_userRiverModel.riverCode];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == 4) {
        ...
        cell.infoTextView.text = _locationModel.addressDetail;
...
- (void)saveButtonClick{
    ...
    dispatch_group_notify(group, dispatch_get_main_queue(), ^{
        ...
        ZLRiverSaveAndSentTaskService *service = [[ZLRiverSaveAndSentTaskService alloc]initWithimgList:self.imageNameArray taskName:self.taskName taskContent:self.taskDesc receiverDepartmentNames:self.taskDepartString receiverDepartmentCodes:self.departCodeString receiverPersonIds:self.peopleCodeString receiverPersonNames:self.taskPeopleString riverCode:_userRiverModel.riverCode longitude:_locationModel.longitude latitude:_locationModel.latitude positionDesc:self.address patrolCode:_patrolCode isUrgent:self.isUrgent];
...
- (UITableView *)mainTableView{
    if (!_mainTableView) {
        ...
        ZLNewReportBottomView *bottomView = [[ZLNewReportBottomView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(mediaView.frame) + 50, Main_Screen_Width, 50 * kScreenHeightRatio) withTitles:@[@"立即下发",@"取消下发"]];
