...
#import "ZLBaiduReportViewController.h"
...
@interface ZLBaiduTrackViewController ()<BMKMapViewDelegate,DYLocationManagerDelegate,UIGestureRecognizerDelegate>
...
@implementation ZLBaiduTrackViewController
...
- (void)loadData{
    ...
    [demand startWithCompletionBlockWithSuccess:^(__kindof YTKBaseRequest * _Nonnull request) {
        ...
        if (modelArray.count > 0) {
            ...
            _demandModel = _riverSourceArray.firstObject;
            //通知主线程刷新
            __weak typeof(self) weakSelf = self;
            dispatch_async(dispatch_get_main_queue(), ^{
                //回调或者说是通知主线程刷新，
                _riverInfoView.demandModel = _demandModel;
                weakSelf.title = _demandModel.rivers_name;
            });
        }else{
            _status = noneRiver;
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                //回调或者说是通知主线程刷新，
                [weakSelf.addOrSubView mas_makeConstraints:^(MASConstraintMaker *make) {
                    make.bottom.equalTo(weakSelf.bottomButtonView.mas_top);
                }];
            });
        }
...
//    _riverInfoView.demandModel = _demandModel;
    ...
    [_riverInfoView mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.equalTo(self.view);
        make.width.mas_equalTo(App_Frame_Width);
//        make.height.mas_greaterThanOrEqualTo(AdaptedHeight(170));
        
        make.height.mas_equalTo(90);
        
//        make.height.mas_lessThanOrEqualTo(AdaptedHeight(200));
    ...
    [_reportAndDown mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.equalTo(_riverInfoView.mas_top).offset(-15);
...
- (void)reportClick{
    ...
    }else if(_status == startRiver || _status == noneRiver){
...
//        [_locationManager geoResult:_locationManager.userLocation];
        __weak typeof(self) weakSelf = self;
        ZLBaiduReportViewController *reportVC = [[ZLBaiduReportViewController alloc]initWithShowFrame:CGRectMake(0, [UIScreen mainScreen].bounds.size.height - App_Frame_Height * 0.9, [UIScreen mainScreen].bounds.size.width, App_Frame_Height * 0.9) ShowStyle:(MYPresentedViewShowStyleFromBottomSpreadStyle) callback:^(id callback) {
            
            if ([callback isEqualToString:@"确定"]) {
                weakSelf.eventAnnotation = [self creatPointWithLocaiton:_locationManager.userLocation.location title:@"上报案件"];
...
- (void)downTaskClick{
    ...
    if (_status == endRiver) {
        ...
        //        [_locationManager geoResult:_locationManager.userLocation];
        __weak typeof(self) weakSelf = self;
        ZLBaiduDownTaskViewController *downVC = [[ZLBaiduDownTaskViewController alloc]initWithShowFrame:CGRectMake(0, [UIScreen mainScreen].bounds.size.height - App_Frame_Height * 0.9, [UIScreen mainScreen].bounds.size.width, App_Frame_Height * 0.9) ShowStyle:(MYPresentedViewShowStyleFromBottomSpreadStyle) callback:^(id callback) {
            
            if ([callback isEqualToString:@"确定"]) {
                weakSelf.eventAnnotation = [self creatPointWithLocaiton:_locationManager.userLocation.location title:@"下发任务"];
...
- (void)startClick{
    ...
    if (_status == noneRiver) {
        [UIAlertView alertWithCallBackBlock:^(NSInteger buttonIndex) {
            
        } title:@"提示" message:@"当前人员没有管辖河道" cancelButtonName:@"确定" otherButtonTitles:nil,nil];
        return;
    }
    ...
    if (_isEndRiver) {
        ...
        } title:@"提示" message: [NSString stringWithFormat:@"当前已选择%@,是否确定开始巡河",self.title] cancelButtonName:@"取消" otherButtonTitles:@"确定", nil];
...
- (void)right_button_event:(UIButton *)sender{
    ...
    if (_isShowMaskview) {
        _riverView = [[ZLRiverSelectView alloc]initWithFrame:CGRectMake(0, 64, App_Frame_Width, App_Frame_Height - 64 + 20)titles:_riverSourceArray importOrNomal:@"0"];
        _riverView.alpha = 0;
        _riverView.importOrNormal = @"0";
