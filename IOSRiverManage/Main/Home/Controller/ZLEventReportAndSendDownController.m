...
@interface ZLEventReportAndSendDownController ()
...
@property (nonatomic, strong) ZLEventReportAndDownView *reportView;
...
@implementation ZLEventReportAndSendDownController
- (void)viewDidLoad {
    [super viewDidLoad];
    ...
    _reportView = [[ZLEventReportAndDownView alloc]initWithFrame:CGRectMake(0, 10, App_Frame_Width, AdaptedHeight(425)) andHasEventNameObject:YES hasLeader:YES];
...
- (BOOL)checkTextFieldContent{
    ...
    if ([_reportView.secondView.eventTextfiled.text isEqualToString:@""]) {
        ...
        } title:@"提示" message:@"必须选择发起类型" cancelButtonName:@"确定" otherButtonTitles:nil, nil];
        
        return NO;
    }

    if ([_reportView.thirdView.eventTextfiled.text isEqualToString:@""]) {
        
        [UIAlertView alertWithCallBackBlock:^(NSInteger buttonIndex) {
            
        } title:@"提示" message:@"必须选择事件对象" cancelButtonName:@"确定" otherButtonTitles:nil, nil];
...
- (void)startServiceWithImageString:(NSString *)imageString {
    ...
    if ([_reportView.secondView.eventTextfiled.text isEqualToString:@"上报"]) {
        ...
        ZLNomalTaskDownService *nomalService = [[ZLNomalTaskDownService alloc]initWithtaskName:_reportView.firstView.eventTextfiled.text taskContent:_reportView.noteTextView.text liable_id:_reportView.liable_id userid:_uid dataImgBase:imageString];
...
- (NSMutableAttributedString *)setTitle{
    ...
    NSMutableAttributedString *title = [[NSMutableAttributedString alloc]initWithString:@"发起事项" attributes:dic];
